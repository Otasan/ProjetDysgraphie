...
public class Lecteur {
    ...
    public Lecteur(File source) throws IOException{
        parser = CSVParser.parse(source, Charset.forName("ISO-8859-1"), CSVFormat.TDF);
    }
    ...
    public List<Point> lire(){
        ...
        for(CSVRecord c:parser){
            if(c.get(" Tip ").equals("1")){
                lesPoints.add(new Point(Integer.parseInt(c.get(" X /100 mm")),Integer.parseInt(c.get(" Y /100 mm")), Integer.parseInt(c.get(" Interv ms")), i));
                i++;
            }
        }
        return lesPoints;
